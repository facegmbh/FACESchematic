    ...
    <>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>planSymbol</code></td><td>object</td><td>How the model is drawn on a <a href="/floorplans">floorplan</a>: <code>{"{ shape, color?, glyph? }"}</code> with shape <code>circle</code> / <code>square</code> / <code>triangle</code> / <code>diamond</code> or a top-view pictogram <code>projector</code> / <code>rack</code> / <code>display</code> / <code>camera</code>, a <code>#rrggbb</code> color, up to two characters printed inside the symbol, an optional <code>imageSrc</code> (a data URL) that replaces all three with your own uploaded picture, and <code>outlineColor</code> / <code>outlineWidthMm</code> for the line around the body (0 = none). Without it, the shape follows the device type (speakers round, subs square, microphones triangles, projectors / cameras / racks / displays as pictograms, other video diamonds) and the color is derived from the id</td></tr>
          <tr><td><code>speakerLoad</code></td><td>object</td><td>Loudspeaker load data for the <a href="/floorplans">line load check</a>: <code>{"{ impedanceOhm, rmsPowerW, tapsW?, profile? }"}</code> — nominal impedance, continuous (RMS) power, the 70&nbsp;V / 100&nbsp;V transformer taps in watts (highest first) and the spectral profile <code>FR</code> / <code>LF</code> / <code>MF</code> / <code>HF</code> / <code>SW</code>. Copied onto placed devices</td></tr>
          <tr><td><code>ampLoad</code></td><td>object</td><td>Amplifier output capability: <code>{"{ channels?, ratedW: { ohm2?, ohm4?, ohm8?, v70?, v100? }, totalRatedW?, maxBurstPerChannelW?, maxBurstTotalW?, maxAvgTotalW?, peakVoltageV?, peakCurrentA?, minImpedanceOhm? }"}</code>. Rated power per channel by load, the shared total and the peak limits; anything omitted is derived from the ratings. Leave <code>v70</code> / <code>v100</code> out when the amplifier has no constant-voltage mode</td></tr>
