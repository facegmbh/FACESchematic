...
import { SYMBOL_INK, glyphColorOn, symbolGlyphOffset, symbolGlyphScale, symbolPrimitives } from "../floorplan";
...
export default function FloorplanSymbolSvg({ group, sizePx, rotationDeg = 0, paddingPx = 0, className, style }: Props) {
  const half = sizePx / 2;
  const contrast = glyphColorOn(group.color);
  const outline = Math.max(0.5, sizePx * 0.04);
  ...
  const picture = group.symbolImageSrc ? (
    ...
    symbolPrimitives(group.shape, sizePx).map((p, i) => {
      ...
      const stroke = p.fill === "color" ? "#00000066" : SYMBOL_INK;
