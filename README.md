...
- **Plan symbol per model** — templates carry a `planSymbol` (shape, color, optional glyph inside the symbol) that groups inherit, so the same loudspeaker looks the same on every plan; models without one are drawn by device type (speakers round, subs square, microphones triangles, video diamonds) in a color derived from the model
