...
- **Lines from the schematic** — on a loudspeaker plan every amplifier channel (speaker-level output) with speakers wired to it is a line; a dropped speaker takes its channel's line automatically and *Sync from schematic* binds and renumbers everything at once. Each line can be wired, renamed, named for the legend and run as Lo-Z / 70 V / 100 V with a tap per speaker
